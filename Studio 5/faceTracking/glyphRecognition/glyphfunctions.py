...
# Description: This file is implementing the functions needed for the glyphtestcv file to fully function. However, as meantion before, 
# the functionality requires further testing as once the webcam detects a shape or glyph, it crashes with an error "input outside of loop array",
# of which has yet to be resolved.

# imports library modules needed to use OpenCV tools and commands
import numpy as np
import cv2
 
def order_points(points): # method created to determine the axis points of a given shape.
# the points are then stored in seperate arrays ready for use.
    ...
    return ordered_points # returns the order points array list
 
def max_width_height(points):# method defined to determine the maximum height and width of the scanned object.
    ...
    return (max_width,max_height)# returns the max width and max height from a given array
